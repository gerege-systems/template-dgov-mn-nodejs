...
export default function GovServicesView() {
  ...
  const q = useQuery({ queryKey: ['gov-services'], queryFn: () => getJSON<GovService[]>('/gov/services') });
  ...
  const apply = async (s: GovService) => {
    ...
    const res = await postJSON<GovApplyResult>('/gov/applications', { service_id: s.id });
    setApplying(null);
    if (!res.ok) {
    ...
    if (res.data?.auto_issued) {
      ...
      setTimeout(() => navigate('/me/references'), 900);
    } else {
      setMsg(`"${s.name}" хүсэлт бүртгэгдлээ. Хянагдсаны дараа мэдэгдэнэ.`);
      setTimeout(() => navigate('/me/applications'), 900);
    }
  };
